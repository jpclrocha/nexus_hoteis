...
CREATE TABLE SCH_NEXUS.TAB_FATURAS 
    ( 
     ID_FATURA          NUMBER (10) 
...
GRANT ALL PRIVILEGES
    ON SCH_NEXUS.TAB_FATURAS TO SCH_NEXUS WITH GRANT OPTION 
;

ALTER TABLE SCH_NEXUS.TAB_FATURAS 
...
CREATE TABLE TAB_QUARTO_COMODIDADE 
    ( 
...
ALTER TABLE TAB_QUARTO_COMODIDADE 
...
ALTER TABLE SCH_NEXUS.TAB_PAGAMENTO 
    ...
    REFERENCES SCH_NEXUS.TAB_FATURAS 
    ( 
     ID_FATURA
    ) 
    NOT DEFERRABLE 
...
JOIN
    SCH_NEXUS.TAB_RESERVAS r ON e.ID_RESERVA = r.ID_RESERVA
JOIN
    SCH_NEXUS.TAB_HOSPEDE h ON r.ID_HOSPEDE_PRINCIPAL = h.ID_HOSPEDE
JOIN
    SCH_NEXUS.TAB_HOSPEDES_ESTADIA he ON e.ID_ESTADIA = he.ID_ESTADIA
JOIN
    SCH_NEXUS.TAB_QUARTOS q ON he.ID_QUARTO = q.ID_QUARTO
WHERE
...
JOIN
    SCH_NEXUS_IDENTITY.TAB_CARGOS c ON f.ID_CARGO = c.ID_CARGO 
;

CREATE OR REPLACE VIEW VW_QUARTO_OCUPADO ( NUM_QUARTO, NOME_TIPO, AND_QUARTO, STS_QUARTO, HOSPEDE_PRINCIPAL, DAT_CHECKOUT_PREVISTA ) AS
SELECT
    q.NUM_QUARTO,
    tq.NOME_TIPO,
    q.AND_QUARTO,
...
LEFT JOIN
    SCH_NEXUS.TAB_RESERVAS r ON e.ID_RESERVA = r.ID_RESERVA
LEFT JOIN
    SCH_NEXUS.TAB_HOSPEDE h ON r.ID_HOSPEDE_PRINCIPAL = h.ID_HOS
ORDER BY
    q.NUM_QUARTO 
;
...
BEFORE INSERT ON SCH_NEXUS.TAB_FATURAS 
FOR EACH ROW 
